...
use token_api::{with_api, CreateRequest, CreateResponse, GetBalanceRequest, GetBalanceResponse,
                TransferRequest, TransferResponse, ReputationRequest, ReputationResponse};
...
fn update_reputation(request: &ReputationRequest) -> Result<ReputationResponse> {
    let token = TokenContract::new();
    
    // TODO: Get sender from authenticated request.
    let reputation = token.update_reputation(&request.get_account().to_owned())?;

    let mut response = ReputationResponse::new();
    response.set_reputation(reputation);
