...
database_schema! {
    pub struct TokenDb {
        ...
        pub reputation_of: Map<String, u64>,
    }
}
...
impl TokenContract {
	...
	let initial_reputation = 0;
        ...
        self.db.reputation_of.insert(&sender, &initial_reputation);
        Ok(())
    }
    ...
    fn get_reputation(&self, addr: &String) -> Result<u64> {
        match self.db.reputation_of.get(addr) {
            Some(b) => Ok(b),
            None => Ok(0),
        }
    }
    ...
    pub fn get_current_reputation(&self, msg_sender: &String) -> Result<u64> {
        self.get_reputation(msg_sender)
    }
    ...
    pub fn update_reputation(&self, msg_sender: &String) -> Result<u64> {
        let initial_reputation = self.get_reputation(msg_sender)?;
        let new_reputation = initial_reputation + 1;
        self.db.reputation_of.insert(msg_sender, &new_reputation);
